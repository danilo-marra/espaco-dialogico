...
# Build project-level validation checks used by workflow scripts.
# Focuses on explicit test and migration requirements for delivery gates.
function Get-ProjectWorkflowValidation {
    param(
        [Parameter(Mandatory = $true)][string]$RepoRoot
    )
    ...
    $hasMigrationsDir = Test-Path -LiteralPath $migrationsDir -PathType Container
    $hasConstitution = Test-Path -LiteralPath $constitutionPath -PathType Leaf

    $hasTestScript = $false
    $hasMigrationCreateScript = $false
    $hasMigrationUpScript = $false
    ...
    if ($hasPackageJson) {
        try {
            ...
            if ($pkg.scripts) {
                ...
                $hasMigrationCreateScript = $scriptNames -contains 'migrations:create'
                $hasMigrationUpScript = ($scriptNames -contains 'migrations:up') -or ($scriptNames -contains 'migrations:up:test')
            }
        } catch {
            # Keep default false flags if package parsing fails.
        }
    }

    $constitutionHasTestGate = $false
    $constitutionHasMigrationGate = $false
    if ($hasConstitution) {
        try {
            $constitutionContent = Get-Content -LiteralPath $constitutionPath -Raw
            $constitutionHasTestGate = $constitutionContent -match 'npm run test|Test-First|tests?'
            $constitutionHasMigrationGate = $constitutionContent -match 'exports\.down = false|migration|forward-only'
        } catch {
            # Keep default false flags on read failures.
        }
    }

    $issues = @()
    if (-not $hasPackageJson) {
        $issues += 'package.json not found at repository root.'
    }
    if (-not $hasTestScript) {
        $issues += "Missing required 'test' script in package.json."
    }
    if (-not $hasMigrationCreateScript) {
        $issues += "Missing required 'migrations:create' script in package.json."
    }
    if (-not $hasMigrationUpScript) {
        $issues += "Missing migration apply script ('migrations:up' or 'migrations:up:test') in package.json."
    }
    if (-not $hasMigrationsDir) {
        $issues += 'Migration directory infra/migrations not found.'
    }
    if (-not $hasConstitution) {
        $issues += 'Constitution file not found at .specify/memory/constitution.md.'
    }
    if ($hasConstitution -and -not $constitutionHasTestGate) {
        $issues += 'Constitution does not explicitly mention test validation gates.'
    }
    if ($hasConstitution -and -not $constitutionHasMigrationGate) {
        $issues += 'Constitution does not explicitly mention migration safety rules.'
    }

    [PSCustomObject]@{
        HAS_PACKAGE_JSON               = $hasPackageJson
        HAS_TEST_SCRIPT                = $hasTestScript
        HAS_MIGRATIONS_CREATE_SCRIPT   = $hasMigrationCreateScript
        HAS_MIGRATIONS_UP_SCRIPT       = $hasMigrationUpScript
        HAS_MIGRATIONS_DIR             = $hasMigrationsDir
        HAS_CONSTITUTION               = $hasConstitution
        CONSTITUTION_TEST_GATE         = $constitutionHasTestGate
        CONSTITUTION_MIGRATION_GATE    = $constitutionHasMigrationGate
