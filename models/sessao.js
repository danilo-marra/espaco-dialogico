...
async function getByAgendamentoIds(agendamentoIds) {
  ...
  try {
    const result = await database.query({
      text: `
        ...
        LEFT JOIN pacientes p ON s.paciente_id = p.id
        LEFT JOIN agendamentos a ON s.agendamento_id = a.id
        WHERE s.agendamento_id = ANY($1::uuid[])
        ORDER BY COALESCE(a.data_agendamento, s.created_at) DESC
      `,
    ...
    console.error("Erro ao buscar sessões por agendamentos:", error);
    throw error;
  }
}
...
async function removeBatchByAgendamentosIds(agendamentoIds) {
  try {
    ...
    const sessoesExcluidas = await database.transaction(async (client) => {
...
async function removeBatchByIds(sessaoIds) {
  try {
    ...
    const totalExcluidas = await database.transaction(async (client) => {
      let total = 0;
...
function formatSessaoResult(row) {
  return {
    ...
    pacienteInfo: {
      ...
      nf_nome_completo: row.paciente_nf_nome_completo,
      nf_telefone: row.paciente_nf_telefone,
      nf_cpf: row.paciente_nf_cpf,
      nf_email: row.paciente_nf_email,
      nf_endereco: row.paciente_nf_endereco,
      nf_dt_entrada: row.paciente_nf_dt_entrada,
...
async function createBatch(sessoesData) {
  ...
  try {
    const totalSessoesCreated = await database.transaction(async (client) => {
      let total = 0;
...
async function updateBatch(sessoesData) {
  ...
  try {
    const totalSessoesUpdated = await database.transaction(async (client) => {
      let total = 0;
